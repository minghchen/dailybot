...
class AgentService:
    ...
    async def _decide_and_enrich(self, original_content: str) -> Optional[str]:
        ...
        prompt = f"""
你是一个严格的智能分析师。你的任务是阅读下面的文章内容，并决定如何处理它。你有三个选择：

1.  **搜索并总结 (SearchAndSummarize)**: 
    - **使用场景**: 当文章内容是**对另一个核心、可搜索的AI相关资源**（如论文、GitHub项目、模型发布会）的**介绍、评论或摘要**时使用。
    - **例子**: 一篇介绍Llama 3技术报告的博客文章。你应该搜索 "Llama 3 technical report"。

2.  **直接总结 (DirectSummarize)**: 
    - **使用场景**: 当文章**本身就是有价值的、信息丰富的AI相关主要来源**时使用。
